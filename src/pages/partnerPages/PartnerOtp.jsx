...
import { partnerOtpVerification, partnerResendOtp } from "../../api/partnerApi";
...
const PartnerOtp = () => {
  ...
  const { partnerEmail, partnerId } = location.state;
  ...
  const onSubmit = async () => {
    try {
      ...
      const res = await partnerOtpVerification(combinedOTP, partnerId);
      if (res?.data?.status) {
        toast.success(res?.data?.message);
        navigate("/partner/login", { state: "Email verified" });
      }
      setLoading(false);
    } catch (error) {
  ...
  const resendOTP = async () => {
    try {
      setLoading(true);
      const res = await partnerResendOtp(partnerEmail);
      if (res?.status === 200) {
        toast.success(res?.data?.message);
        setCountDown(30);
            ...
            <div className="mx-auto flex w-full max-w-md flex-col space-y-16">
              <div className="flex flex-col items-center justify-center text-center space-y-2">
                ...
                <div className="flex flex-row text-sm font-medium text-gray-400">
                  <p>We have sent a code to your email {partnerEmail}</p>
                </div>
              </div>
